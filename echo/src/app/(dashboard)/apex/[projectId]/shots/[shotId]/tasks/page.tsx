...
export default function ShotTasksPage({
  ...
  useEffect(() => {
    params.then((p) => {
      ...
      setShotId(p.shotId)
      loadTasks(p.projectId, p.shotId)
    })
  }, [params])

  async function loadTasks(projId: string, sId: string) {
    try {
      setIsLoading(true)
      ...
      if (error) {
        console.error('Error loading shot tasks:', error)
        setTasks([])
        return
      }
    ...
    <div className="p-6">
      {isLoading ? (
        ...
        <div className="rounded-md border border-zinc-800 bg-zinc-950/70 p-6">
          ...
          <p className="mt-2 text-sm text-zinc-400">No tasks linked to this shot yet.</p>
        </div>
      ) : (
        <EntityTable
          columns={columns}
