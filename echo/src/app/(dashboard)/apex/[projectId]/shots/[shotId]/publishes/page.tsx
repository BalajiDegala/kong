...
export default function ShotPublishesPage({
  params,
}: {
  ...
  useEffect(() => {
    params.then((p) => {
      ...
      setShotId(p.shotId)
      loadPublishes(p.projectId, p.shotId)
    })
  }, [params])

  async function loadPublishes(projId: string, sId: string) {
    try {
      setIsLoading(true)
      ...
      const [publishesResult, nextStatusNames, nextTagNames] = await Promise.all([
        supabase
          ...
          .eq('entity_type', 'shot')
          .eq('entity_id', Number.isNaN(shotIdNum) ? sId : shotIdNum)
      ...
      if (error) {
        console.error('Error loading shot publishes:', error)
        setPublishes([])
        return
      }
    ...
    <>
      <CreatePublishedFileDialog
        ...
        onOpenChange={(open) => {
          ...
          if (!open) loadPublishes(projectId, shotId)
        }}
        projectId={projectId}
        ...
        {isLoading ? (
          ...
          <div className="rounded-md border border-zinc-800 bg-zinc-950/70 p-6">
            ...
            <p className="mt-2 text-sm text-zinc-400">
              No published files linked to this shot yet.
            </p>
            <button
