...
export default function ShotVersionsPage({
  ...
  useEffect(() => {
    params.then((p) => {
      ...
      setShotId(p.shotId)
      loadVersions(p.projectId, p.shotId)
    })
  }, [params])

  async function loadVersions(projId: string, sId: string) {
    try {
      setIsLoading(true)
      ...
      if (error) {
        console.error('Error loading shot versions:', error)
        setVersions([])
        return
      }
    ...
    <>
      <UploadVersionDialog
        ...
        onOpenChange={(open) => {
          ...
          if (!open) loadVersions(projectId, shotId)
        }}
        projectId={projectId}
      ...
      <div className="p-6">
        {isLoading ? (
          ...
          <div className="rounded-md border border-border bg-background/70 p-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">No versions linked to this shot yet.</p>
            <button
