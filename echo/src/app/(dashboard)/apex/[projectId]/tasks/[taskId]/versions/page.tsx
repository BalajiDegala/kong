...
export default function TaskVersionsPage({
  ...
  useEffect(() => {
    params.then((p) => {
      ...
      setTaskId(p.taskId)
      loadVersions(p.projectId, p.taskId)
    })
  }, [params])

  async function loadVersions(projId: string, tId: string) {
    try {
      setIsLoading(true)
      const supabase = createClient()
      const taskIdNum = Number(tId)
      const taskIdFilter = Number.isNaN(taskIdNum) ? tId : taskIdNum
      ...
      const entityType = taskData?.entity_type
      if (
        (entityType === 'asset' || entityType === 'shot' || entityType === 'sequence') &&
        taskData?.entity_id !== null &&
        taskData?.entity_id !== undefined
      ) {
        setTaskEntityType(entityType)
      ...
      <div className="p-6">
        {isLoading ? (
          ...
          <div className="rounded-md border border-border bg-background/70 p-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">No versions linked to this task yet.</p>
            <button
