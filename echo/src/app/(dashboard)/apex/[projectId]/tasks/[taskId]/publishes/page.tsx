...
export default function TaskPublishesPage({
  params,
}: {
  ...
  useEffect(() => {
    params.then((p) => {
      ...
      setTaskId(p.taskId)
      loadPublishes(p.projectId, p.taskId)
    })
  }, [params])

  async function loadPublishes(projId: string, tId: string) {
    try {
      setIsLoading(true)
      const supabase = createClient()
      const taskIdNum = Number(tId)
      ...
      const [publishesResult, nextStatusNames, nextTagNames] = await Promise.all([
        supabase
          ...
          .eq('task_id', Number.isNaN(taskIdNum) ? tId : taskIdNum)
      ...
      if (error) {
        console.error('Error loading task publishes:', error)
        setPublishes([])
        return
      }
    ...
    <>
      <CreatePublishedFileDialog
        ...
        onOpenChange={(open) => {
          ...
          if (!open) loadPublishes(projectId, taskId)
        }}
        projectId={projectId}
        defaultEntityType="task"
        defaultEntityId={taskId}
      ...
      <div className="p-6">
        {isLoading ? (
          ...
          <div className="rounded-md border border-border bg-background/70 p-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">No published files linked to this task yet.</p>
            <button
