...
export default function SequenceAssetsPage({
  params,
}: {
  ...
  useEffect(() => {
    params.then((p) => {
      ...
      loadAssets(p.projectId, p.sequenceId)
    })
  }, [params])

  async function loadAssets(projId: string, seqId: string) {
    try {
      setIsLoading(true)
      const supabase = createClient()
      const sequenceIdNum = Number(seqId)
      const sequenceIdFilter = Number.isNaN(sequenceIdNum) ? seqId : sequenceIdNum
      ...
      if (sequenceRow?.code) {
        ...
        query = query.eq('sequence_id', sequenceIdFilter)
      }
      ...
      console.error('Error loading sequence assets:', error)
      setAssets([])
    } finally {
      setIsLoading(false)
    }
      ...
      <div className="p-6">
        {isLoading ? (
          ...
          <div className="rounded-md border border-border bg-background/70 p-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">No assets linked to this sequence yet.</p>
            <button
